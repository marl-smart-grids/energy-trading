...
np.random.seed(7)
random.seed(7)
# The main difference between stochastic ADL case and non stochastic ADL case is that, in non stochastic case the ADL demands are same for all the agents 
# and same ADL demands are given at the start of each day, but in stochastic ADL the ADL demands are variable. They vary among agents and vary for each agent every day 
...
class DQN_Agent:
    ...
    #Converts Decimal value to ternary 
    ...
    # distribution. We get the lambda value for the poisson distribution by preprocessing a data set.
    def get_renewable(self,time):
        energy = np.random.poisson(lam=self.lam[time-1],size=1)
        energy = min([10, energy]) # clipping the value so that it can't exceede 10
    ...
    # to get the range of possible pricing actions. The design constraints give us the range of electricty values that can be traded. Negative values means that a grid is buying 
    ...
    # convert a pricing action index to the respective price quoted and energy to be traded
    ...
    # Takes a pricing pricing action based on the available pricing action indexes using an epsilon greedy policy(either choose a random action with epsilon proabilty
    ...
    # Randomisation for ADL state also happens here. At the start of the day(time == 4) the ADL demands are randomly selected 
